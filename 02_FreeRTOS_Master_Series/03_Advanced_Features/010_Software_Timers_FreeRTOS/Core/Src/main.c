...
int main(void)
{
  ...
  MX_USART2_UART_Init();
  /* USER CODE BEGIN 2 */
  printLog("\r\n--- 010 Software Timer Projesi Baslatildi ---\r\n");

  if (osTimerStart(myTimer01Handle, 1000) == osOK)
  {
      printLog("Zamanlayici basariyla baslatildi (1 sn periyot).\r\n");
  }
...
{
  /* USER CODE BEGIN 5 */
	char *heartbeatMsg = "--- [SYSTEM] Default Task calisiyor (Heartbeat) ---\r\n";
  /* Infinite loop */
  for(;;)
  {
...
void myTimer01_Callback(void const * argument)
{
  /* USER CODE BEGIN myTimer01_Callback */
	  ...
	  printLog("Timer Callback: 1 saniye doldu, LED durumu degisti.\r\n");
